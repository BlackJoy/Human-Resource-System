...
<html>
  ...
  <body>
    ...
    <form id="accountUpdateForm" action="account_update.action" method="post">
    <input type="hidden" id="orgId" name="orgId" value="<s:property value="orgId"/>" />
    <input type="hidden" id="empId" name="empId" value="<s:property value="empId"/>" />
    <input type="hidden" id="pageInd
    ex" name="pageIndex" value="<s:property value="pageIndex"/>" />
    <input type="hidden" id="pageIndex" name="user.id" value="<s:property value="user.id"/>" />
    <input type="hidden" id="pageIndex" name="user.employeeId" value="<s:property value="user.employeeId"/>" />
    ...
    <table border="0" width="100%" style="background: rgb(240, 240, 240); font-size: 12px;" cellpadding="0" cellspacing="2">
    	<tr height="30px;">
    		...
    		<td width="30%"><input type="text" name="user.username" id="username" value="<s:property value="user.username" />" /></td>
    	...
    	<tr height="30px;">
    		...
    		<td><input type="password" name="user.password" id="password" value="<s:property value="user.password" />" /></td>
    		<td align="right">确认密码：</td>
    		<td><input type="password" name="repassword" id="repassword" value="<s:property value="user.password" />" /></td>
    		<td>&nbsp;</td>
    	</tr>
    	<tr height="30px;">
    		<td align="right"><span style="color:red;">*</span>账户状态：</td>
    		<td>
    			<input type="radio" name="user.enable" <s:if test="user.enable == 0">checked="checked"</s:if> value="0" />注销&nbsp;&nbsp;
    			<input type="radio" name="user.enable" <s:if test="user.enable == 1">checked="checked"</s:if> value="1" />启用
    		</td>
    		<td align="right">&nbsp;</td>
    		<td>&nbsp;</td>
    		<td>&nbsp;</td>
