...
<html>
	...
	$(document).ready(function(){
		$("#doApprove").click(function(){
			...
			if (count <= 0) {
				...
				alert("只能对单个待传阅项目进行审批");
			} else {
				$("#reviewApproveForm")[0].submit();
			}
		});
	});
  ...
  <body>
    	...
    	<tr>
    		...
    		<table border="0" cellpadding="0" cellspacing="0">
    			<tr>
		    		...
		    		<td height="26px" style="background: url(images/yq_bg.gif) repeat-x; font-size:12px; font-weight: bold; color:#333;">&nbsp;待传阅项目&nbsp;</td>
    ...
    <table border="0" width="100%" style="background: rgb(192, 212, 240);" cellpadding="0" cellspacing="0">
    	<tr>
    		<td height="30px" valign="middle" style="padding-left:30px;">
    			<input id="doApprove" type="button" value="项目查阅" onclick="doAdd()" />&nbsp;&nbsp;
    		</td>
    	</tr>
    </table>
    <form name="reviewApproveForm" id="reviewApproveForm" action="approve_toReviewApprove.action" method="post">
    ...
    <table border="0" width="100%" style="background: #fafafa; font-size:12px;" cellpadding="0" cellspacing="1">
    	...
    	<s:iterator var="approve" value="#request.reviewApprove_page.list" status="i">
    		<s:if test="#i.odd">
			...
			<tr style="background: rgb(248, 248, 248); height:22px;">
				...
				<td align="center"><input class="ids" name="id" type="checkbox" value="<s:property value="ID"/>" <s:if test="STATUS != 1">disabled="disabled" </s:if>/></td>
				<td align="center"><s:property value="APPROVENAME"/></td>
    	...
    	<s:iterator begin="#rowCount+1" end="#request.reviewApprove_page.pageSize" status="i">
    ...
    <s:set name="pageCount" value="(#request.reviewApprove_page.rowCount+#request.reviewApprove_page.pageSize-1)/#request.reviewApprove_page.pageSize"></s:set>
    <table border="0" width="100%" height="24px" style="background: rgb(192, 212, 240);" cellpadding="0" cellspacing="0">
    	<tr>
    		<td align="right">
    			共有&nbsp;<s:property value="#request.reviewApprove_page.rowCount" />&nbsp;条信息&nbsp;&nbsp;
    			<s:property value="#request.reviewApprove_page.pageSize" />条/页&nbsp;&nbsp;
    			页次：<span style="color:red;"><s:property value="#request.reviewApprove_page.pageIndex" /></span>/<s:property value="#pageCount" />页&nbsp;&nbsp;
    			<s:if test="#request.reviewApprove_page.pageIndex == 1">首页</s:if>
    			<s:else><a class="page_l" href="javascript:go(1)">[首页]</a></s:else>
    			<s:if test="#request.reviewApprove_page.pageIndex == 1">上一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.reviewApprove_page.pageIndex-1" />)">[上一页]</a></s:else>
    			<s:if test="#request.reviewApprove_page.pageIndex >= #pageCount">下一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.reviewApprove_page.pageIndex+1" />)">[下一页]</a></s:else>
    			<s:if test="#request.reviewApprove_page.pageIndex >= #pageCount">尾页</s:if>
