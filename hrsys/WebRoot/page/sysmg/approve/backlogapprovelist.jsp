...
<html>
  ...
  <body>
    	...
    	<tr>
    		...
    		<table border="0" cellpadding="0" cellspacing="0">
    			<tr>
		    		...
		    		<td height="26px" style="background: url(images/yq_bg.gif) repeat-x; font-size:12px; font-weight: bold; color:#333;">&nbsp;待审批项目&nbsp;</td>
    ...
    <table border="0" width="100%" style="background: #fafafa; font-size:12px;" cellpadding="0" cellspacing="1">
    	...
    	<s:iterator var="approve" value="#request.backlogApprove_page.list" status="i">
    		<s:if test="#i.odd">
    	...
    	<s:iterator begin="#rowCount+1" end="#request.backlogApprove_page.pageSize" status="i">
    ...
    <s:set name="pageCount" value="(#request.backlogApprove_page.rowCount+#request.backlogApprove_page.pageSize-1)/#request.backlogApprove_page.pageSize"></s:set>
    <table border="0" width="100%" height="24px" style="background: rgb(192, 212, 240);" cellpadding="0" cellspacing="0">
    	<tr>
    		<td align="right">
    			共有&nbsp;<s:property value="#request.backlogApprove_page.rowCount" />&nbsp;条信息&nbsp;&nbsp;
    			<s:property value="#request.backlogApprove_page.pageSize" />条/页&nbsp;&nbsp;
    			页次：<span style="color:red;"><s:property value="#request.backlogApprove_page.pageIndex" /></span>/<s:property value="#pageCount" />页&nbsp;&nbsp;
    			<s:if test="#request.backlogApprove_page.pageIndex == 1">首页</s:if>
    			<s:else><a class="page_l" href="javascript:go(1)">[首页]</a></s:else>
    			<s:if test="#request.backlogApprove_page.pageIndex == 1">上一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.backlogApprove_page.pageIndex-1" />)">[上一页]</a></s:else>
    			<s:if test="#request.backlogApprove_page.pageIndex >= #pageCount">下一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.backlogApprove_page.pageIndex+1" />)">[下一页]</a></s:else>
    			<s:if test="#request.backlogApprove_page.pageIndex >= #pageCount">尾页</s:if>
