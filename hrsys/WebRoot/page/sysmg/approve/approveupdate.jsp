...
<html>
  ...
  <body>
    ...
    <form name="approveUpdateFrm" id="approveUpdateFrm" action="approve_doUpdate.action" method="post">
    <input type="hidden" name="approveFlow" id="approveFlow" />
    <input type="hidden" name="approve.id" id="approveId" value="<s:property value="approve.id" />" />
    <table border="0" width="100%" style="background: rgb(240, 240, 240); font-size: 12px;" cellpadding="0" cellspacing="5">
    	<tr>
    		...
    		<td><input type="text" name="approve.name" id="approveName" value="<s:property value="approve.name" />" /></td>
    	</tr>
    	<tr>
    		<td width="30%" align="right"><span style="color:red;">*</span>审批期限（天）：</td>
    		<td><input type="text" name="approve.timeLimit" id="approveTimeLimit" value="<s:property value="approve.timeLimit" />" /></td>
    	</tr>
    	<tr>
    		...
    		<td>
    			<select name="approve.approveContent">
    				...
    				<s:iterator var="ac" value="#application.approveContent">
    					<option value="<s:property value="action" />" <s:if test="approve.approveContent==#ac.action">selected="selected"</s:if>><s:property value="name" /></option>
    				</s:iterator>
    			</select>
    		</td>
    	</tr>
    ...
    <div id="flow" style="padding: 20px; font-size: 12px; overflow: auto;">
    	<div id="flow_view" style="width:100%;">
	    	...
	    	<div class="flow_item" style="width: 198px; float: left; display:inline; position: relative; margin-left: 12px;" flowOrder="<s:property value="#flowNode.key" />">
	    		<div class="flow_add" style="width: 15px; heigth: 15px; text-align: center; line-height: 15px; position: absolute; right: -15px; top:20px; cursor: pointer;" flowOrder="<s:property value="#flowNode.key" />">+</div>
		    	...
		    	<div class="flow_node" style="width: 120px; padding: 5px; border: 1px solid #999; float: left; position: relative;">
		    		...
		    		<s:if test="#flow.approveType == 1">
		    		<div class="flow_approve" style="width: 108px; padding: 5px; border: 1px solid #999;">
		    			<div style="text-align: center;">审批</div>
		    			<div style="padding-top: 5px;">
		    				<input class="approverName" name="approverName" type="text" style="width: 80px;" value="<s:property value="#flow.approverName" />" /><input class="approverId" name="approverId" type="hidden" value="<s:property value="#flow.approverId" />"><input class="approveType" name="approveType" type="hidden" value="1">
		    		...
		    		<div class="flow_read" style="width: 108px; padding: 5px; margin-top: 5px; border: 1px solid #999;">
		    			...
		    			<div style="padding-top: 5px;">
		    				<input class="approverName" name="approverName" type="text" style="width: 80px;" value="<s:property value="#flow.approverName" />" /><input class="approverId" name="approverId" type="hidden" value="<s:property value="#flow.approverId" />"><input class="approveType" name="approveType" type="hidden" value="2">
		    				<div class="choose_approver" style="width:18px; height:18px; float:right; cursor: pointer;"><img src="images/choose.jpg" /></div>
		    			</div>
		    		</div>
		    		</s:else>
