...
<html>
  <head>
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function(){
			...
			$("#renyuanhuizong").click(function(){
				...
				else 
				{
					...
					$("#kqTime").val(rtnValue);
					$("#kaoqinForm").attr("action", "kaoqinmonth_renyuanhuizong.action");
					$("#kaoqinForm")[0].submit();
					}
				}		
			});
			...
			$("#bumenhuizong").click(function(){
					...
					$("#kqTime").val(rtnValue);
					$("#kaoqinForm").attr("action", "kaoqinmonth_bumenhuizong.action");
					$("#kaoqinForm")[0].submit();
					}
			...
			$("#huizongchaxun").click(function(){
					...
					$("#kaoqinForm").attr("action", "kaoqinmonth_huizongchaxun.action");
					$("#kaoqinForm")[0].submit();
					}
		...
		function go(pageIndex){
			...
			self.location.href="<%=basePath%>kaoqinmonth_list.action?pageIndex="+pageIndex+"&orgId="+oid;
		}
	</script>
  ...
  <body>
    ...
    <table border="0" width="100%" style="background: rgb(192, 212, 240);" cellpadding="0" cellspacing="0">
    	<tr>
    		<td height="30px" valign="middle" style="padding-left:30px;">
    			...
    			<input id="renyuanhuizong" type="button" value="人员月度汇总" <ct:btnEnable id="1.7.2.1" /> />&nbsp;&nbsp;
    			<input id="bumenhuizong" type="button" value="部门月度汇总" <ct:btnEnable id="1.7.2.2" /> />&nbsp;&nbsp;
    			<input id="huizongchaxun" type="button" value="查看月度汇总记录" <ct:btnEnable id="1.7.2.2" /> />&nbsp;&nbsp;
    		</td>
    	</tr>
    </table>
    ...
    <table border="0" width="100%" style="background: #fafafa; font-size:12px;" cellpadding="0" cellspacing="1">
    	...
    	<s:iterator var="account" value="#request.kaoqinmonth_list_page.list" status="i">
    		<s:if test="#i.odd">
    	...
    	<s:iterator begin="#rowCount+1" end="#request.kaoqinmonth_list_page.pageSize" status="i">
    ...
    <s:set name="pageCount" value="(#request.kaoqinmonth_list_page.rowCount+#request.kaoqinmonth_list_page.pageSize-1)/#request.kaoqinmonth_list_page.pageSize"></s:set>
    <table border="0" width="100%" height="24px" style="background: rgb(192, 212, 240); font-size:12px;" cellpadding="0" cellspacing="0">
    	<tr>
    		<td align="right">
    			共有&nbsp;<s:property value="#request.kaoqinmonth_list_page.rowCount" />&nbsp;条信息&nbsp;&nbsp;
    			<s:property value="#request.kaoqinmonth_list_page.pageSize" />条/页&nbsp;&nbsp;
    			页次：<span style="color:red;"><s:property value="#request.kaoqinmonth_list_page.pageIndex" /></span>/<s:property value="#pageCount" />页&nbsp;&nbsp;
    			<s:if test="#request.kaoqinmonth_list_page.pageIndex == 1">首页</s:if>
    			<s:else><a class="page_l" href="javascript:go(1)">[首页]</a></s:else>
    			<s:if test="#request.kaoqinmonth_list_page.pageIndex == 1">上一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.kaoqinmonth_list_page.pageIndex-1" />)">[上一页]</a></s:else>
    			<s:if test="#request.kaoqinmonth_list_page.pageIndex >= #pageCount">下一页</s:if>
    			<s:else><a class="page_l" href="javascript:go(<s:property value="#request.kaoqinmonth_list_page.pageIndex+1" />)">[下一页]</a></s:else>
    			<s:if test="#request.kaoqinmonth_list_page.pageIndex >= #pageCount">尾页</s:if>
