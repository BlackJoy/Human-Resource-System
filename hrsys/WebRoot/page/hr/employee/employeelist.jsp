	...
	<script type="text/javascript" language="javascript">
		$(document).ready(function(){
			$("#doAdd").click(function(){
					...
					$("#accountForm").attr("action", "employee_toAdd.action");
			...
			$("#doUpdate").click(function(){
				...
				if (count <= 0) {
					...
					$("#accountForm").attr("action", "employee_toUpdate.action");
					$("#accountForm")[0].submit();
				}
			});
			$("#doDelete").click(function(){
				...
				if (count <= 0) {
					...
					if (confirm("是否确定删除该员工信息？")) {
						$("#accountForm").attr("action", "employee_delete.action");
						$("#accountForm")[0].submit();
					}
				}
			});
		...
		function toDiaoDong()
		{
		   $("#accountForm").attr("action", "employee_toDiaoDong.action");
		   $("#accountForm")[0].submit();
		}
		function toXiangQing()
		{
		    $("#accountForm").attr("action", "employee_toXiangQing.action");
			$("#accountForm")[0].submit();	
		}
    ...
    <table border="0" width="100%" style="background: rgb(192, 212, 240);" cellpadding="0" cellspacing="0">
    	<tr>
    		<td height="30px" valign="middle" style="padding-left:30px;">
    			<input id="doAdd" type="button" value="增加" <ct:btnEnable id="1.3.2.1" /> />&nbsp;&nbsp;
    			<input id="doUpdate" type="button" value="修改" <ct:btnEnable id="1.3.2.2" /> />&nbsp;&nbsp;
    			<input id="doDelete" type="button" value="删除" <ct:btnEnable id="1.3.2.3" /> />&nbsp;&nbsp;
    		</td>
    	</tr>
    </table>
    ...
    <table border="0" width="100%" style="background: #fafafa; font-size:12px;" cellpadding="0" cellspacing="1">
    	...
    	<s:iterator var="account" value="#request.employee_list_page.list" status="i">
			...
			<tr style="background: rgb(248, 248, 248); height:22px;">
				...
				<td align="center">
				<input id="XiangQing" type="button" value="详情" onclick="toXiangQing()"<ct:btnEnable id="1.3.2.4" /> />
				<input id="DiaoDong" type="button" value="调动" onclick="toDiaoDong()"<ct:btnEnable id="1.3.2.5" /> />
				</td>
    		</tr>
